...
1. **Deterministic exact matching first.** The user's preserved wording (`entity_id` strings and friendly names as they appear in the query or condensed task) must be tried through exact deterministic lookups before any translated, condensed, summarized, or paraphrased text. An `entity_id` proposed by an agent LLM is treated strictly as validation input to this deterministic stage, never as a trusted selection. Hybrid scoring may not override an exact deterministic match for the same query.
...
5. **Hybrid matcher fallback.** Only if no exact deterministic candidate is found may the executor fall back to the hybrid matcher (fuzzy, embedding, phonetic, area-aware scoring). The fallback must pass `preferred_domains` derived from the executor's allowed domains.
