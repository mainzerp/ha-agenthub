...
class TestTimerExecutor:
    ...
    async def test_create_recurring_reminder_alarm_like_reroutes_to_internal_alarm(self):
        """Alarm-like recurring reminders reroute to internal set_datetime recurrence flow."""
        from unittest.mock import patch as _patch

        scheduler = MagicMock()
        scheduler.schedule = AsyncMock(return_value="alarm-reroute-1")
        ha_client = MagicMock()
        ...
        assert result["success"] is True
        assert result["metadata"]["scheduler_alarm_id"] == "alarm-reroute-1"
        scheduler.schedule.assert_awaited_once()
        kwargs = scheduler.schedule.await_args.kwargs
        assert kwargs["kind"] == "alarm"
        assert kwargs["logical_name"] == "Wecker"
        recurrence = kwargs["payload"]["recurrence"]
        assert recurrence["freq"] == "weekly"
    ...
    async def test_create_recurring_reminder_non_alarm_keeps_calendar_path(self):
        """Non-alarm recurring reminders keep calendar create_event behavior."""
        scheduler = MagicMock()
        scheduler.schedule = AsyncMock(return_value="alarm-should-not-run")
        ha_client = MagicMock()
        ha_client.call_service = AsyncMock()
        entity_index = MagicMock()
        ...
        ha_client.call_service.assert_awaited_once_with(
            ...
            "calendar.family",
            {
                ...
                "start_date_time": "2026-04-27 08:00:00",
                "rrule": "FREQ=DAILY",
                "end_date_time": "2026-04-27 08:00:00",
            },
        )
        scheduler.schedule.assert_not_awaited()

    async def test_create_recurring_reminder_alarm_like_invalid_rrule_fails_without_side_effects(self):
        """Alarm-like recurring reminders with unsupported RRULE fail and do not schedule or hit calendar."""
        from unittest.mock import patch as _patch
        ...
        assert "Unsupported RRULE frequency" in result["speech"]
        scheduler.schedule.assert_not_awaited()
        ha_client.call_service.assert_not_awaited()
