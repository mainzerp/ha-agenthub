...
@.github/instructions/prime-directives.md
...
- Spawns subagents (via the Task tool, by referencing the agent name from `.claude/agents/`) for complex multi-step tasks
...
- Subagents are defined in `.claude/agents/<name>.md` and run in a fresh context window every time.
- Invoke a subagent by name, e.g.:
  `> Use the research subagent to investigate <topic>.`
  `> Use the planner subagent to plan based on docs/SubAgent/X_ANALYSIS.md.`
  `> Use the implementer subagent to implement docs/SubAgent/X_PLAN.md.`
- Auto-delegation may also pick a subagent based on its frontmatter `description`; explicit invocation is preferred for the mandatory workflow.
- Models are pinned in each agent file:
    - `research`, `planner` -> `opus`
    - `implementer` -> `sonnet`

## Plan Approval (substitute for `plan_review`)

Claude Code has no dedicated plan-review UI. The orchestrator MUST:
...
Optional reinforcement: the user may toggle Plan Mode (Shift+Tab) to prevent any write tool from running until they accept; this is user-initiated and does not replace the explicit chat question above.

## Final Confirmation (substitute for `ask_user`)
...
5. **Always invoke subagents through the dedicated agent files** (`.claude/agents/{research,planner,implementer}.md`). Do not perform research, planning, or implementation directly from the orchestrator.
6. **If a subagent reports it cannot find its agent file**, confirm `.claude/agents/<name>.md` exists and is committed.
